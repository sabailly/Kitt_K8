...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/11/08 09:11:57 by abaurens          #+#    #+#             */
/*   Updated: 2018/11/08 09:12:21 by abaurens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	ft_strrchr_test(void)
{
	...
	c = 0;
	success = 0;
	printf(KYEL"testing ft_strrchr()... "KNRM);
	while (c < 255)
	{
		expected = strrchr(g_s, c);
		if (expected == (result = ft_strrchr(g_s, c)))
			success++;
		else
